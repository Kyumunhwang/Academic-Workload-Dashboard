  ...
  loading: () => (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center min-h-[500px]">
        ...
        <p className="mt-4 text-sm font-semibold text-slate-700">Loading Student Academic Calendar...</p>
      </div>
    </div>
  ),
});
...
export default function StudentCalendarPage() {
  return (
    <main className="min-h-screen py-4">
      {/* Student View Header & Grade Selector */}
      <GradeSelector variant="student" />
      {/* Read-Only Calendar Board for Students */}
      <CalendarBoard readOnly={true} />
    </main>
  );
}
