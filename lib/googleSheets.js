...
let sampleMockData = [
  ...
  },
  {
    ...
    rowIndex: 3,
    date: '2026-09-10',
    rawGrade: 'Grade 6, Grade 7',
    grade: 'Grade 6, Grade 7',
    grades: ['Grade 6', 'Grade 7'],
    subject: 'Spanish (Elective)',
    rawType: 'Project',
    type: 'Project',
    description: 'Spanish cultural presentation (Leveled / Elective)',
  },
  {
    id: 'mock-3',
    rowIndex: 4,
...
import { parseGrades, normalizeGrade } from './gradeUtils';
export { parseGrades, normalizeGrade };
  ...
  try {
    ...
    const parsedEvents = dataRows
      .map((row, index) => {
        ...
        const parsedGrades = parseGrades(rawGrade);
        const standardizedGrade = normalizeGrade(rawGrade);
        ...
        return {
          ...
          grades: parsedGrades.length > 0 ? parsedGrades : (standardizedGrade ? [standardizedGrade] : []),
...
export async function updateCalendarEvent({
  ...
  const clientInfo = getSheetsClient();
  const normalizedGradeVal = Array.isArray(grade)
    ? grade.join(', ')
    : grade !== undefined
  ...
  if (!clientInfo) {
    sampleMockData = sampleMockData.map((item) => {
      if (item.id === id || item.rowIndex === rowIndex) {
        const nextGrade = normalizedGradeVal !== undefined ? normalizedGradeVal : item.grade;
        return {
          ...item,
          date: date || item.date,
          grade: nextGrade,
          grades: parseGrades(nextGrade),
  ...
  try {
    ...
    if (normalizedGradeVal !== undefined) currentRow[colMap.grade] = normalizedGradeVal;
